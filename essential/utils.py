...
def generate_audio_from_excel(excel_path, output_folder):
    os.makedirs(BASE_DIR, output_folder, exist_ok=True)
    df = pd.read_excel(excel_path)
    if "eng" not in df.columns:
        raise KeyError(
            "The Excel file must contain a column named 'eng' for English words."
        )
    for index, row in df.iterrows():
        ...
        try:
            ...
            audio_file_path = os.path.join(output_folder, f"{english_word}.mp3")
            tts.save(audio_file_path)
            print(f"Audio generated: {audio_file_path}")
        except Exception as e:
